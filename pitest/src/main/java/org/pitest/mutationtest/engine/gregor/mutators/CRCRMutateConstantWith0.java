...
public class CRCRMutateConstantWith0 implements MethodMutatorFactory {
  ...
  private final class NegateVariableVisitor extends MethodVisitor {
    ...
    private <T extends Number> boolean shouldMutate(final T constant,
      ...
      final MutationIdentifier mutationId = this.context.registerMutation(
          CRCRMutateConstantWith0.this, "Bug fixed: CRCR replace with Zero, Substituted " + constant + " with "
              + replacement);

      return true; //this.context.shouldMutate(mutationId);
    }
    ...
    private boolean shouldMutate() {
        final MutationIdentifier mutationId = this.context.registerMutation(new CRCRMutateConstantWith0(),
  ...
  public String toString() {
    return "CRCR_VARIABLE_MUTATOR_TO_ZERO";
  }
